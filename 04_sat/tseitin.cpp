...
using AtomSubs = std::map<std::string, int>;

using NLiteral = int;
using NClause = std::vector<NLiteral>;
using CNF = std::vector<NClause>;

int tseitin(FormulaPtr, CNF&, AtomSubs&, int&);
int tseitinF(CNF& cnf, AtomSubs& atomSubs, int& slot) { cnf.push_back({-slot}); return slot++; }
int tseitinT(CNF& cnf, AtomSubs& atomSubs, int& slot) { cnf.push_back({slot});  return slot++; }
int tseitinA(Atom a, CNF& cnf, AtomSubs& atomSubs, int& slot) {
    if(atomSubs.find(a.name) == end(atomSubs))
        atomSubs[a.name] = slot++;
    return atomSubs[a.name];
}
int tseitinN(Not n, CNF& cnf, AtomSubs& atomSubs, int& slot) {
    int slotSub = tseitin(n.sub, cnf, atomSubs, slot);

    cnf.push_back({-slot, -slotSub});
    cnf.push_back({slot, slotSub});

    return slot++;
}
int tseitinAnd(FormulaPtr l, FormulaPtr r, CNF& cnf, AtomSubs& atomSubs, int& slot) {
    int slotL = tseitin(l, cnf, atomSubs, slot);
    int slotR = tseitin(r, cnf, atomSubs, slot);

    cnf.push_back({-slot, slotL});
    cnf.push_back({-slot, slotR});
    cnf.push_back({-slotL, -slotR, slot});

    return slot++;
}
int tseitinOr(FormulaPtr l, FormulaPtr r, CNF& cnf, AtomSubs& atomSubs, int& slot) {
    int slotL = tseitin(l, cnf, atomSubs, slot);
    int slotR = tseitin(r, cnf, atomSubs, slot);
